...
export async function searchVendors(query: string, page: number = 1, limit: number = 10) {
  const response = await axiosClient.get<ApiResponse<VendorSearchResult[]>>(
    `/admin/vendors/search?query=${query}&page=${page}&limit=${limit}`
  )
